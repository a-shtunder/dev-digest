...
- **Merge note (lesson-2-lab/skills → main):** lab2 built a parallel, full-`Finding[]` findings query for the same route (see the superseded Codebase Patterns entry above). Kept this counts-based query instead — lab2's version and the client-side `FindingsCountChips`/`FindingsHoverCard`/`FindingPreview` components weren't merged in.
